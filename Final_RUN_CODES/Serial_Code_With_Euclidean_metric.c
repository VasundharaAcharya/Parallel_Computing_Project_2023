...
/* This code uses KMedoids implemented with euclidean_distance distance metric to compute the cluster assignments.
...
//Since I am using all gather and updating the medoids, I dont require the below updation logic.
   ...
   /* for (int a=0; a<K;a++){
    ...
    if (world_rank == 0) {
        ...
        printf("Total time taken to run K-Medoids on %d genes with %d clusters with euclidean distance metric is %e seconds.\n", Genes, K, (endtime-starttime)/512000000.0f);
        printf("Total clockcycles taken to run K-Medoids on %d genes with %d clusters with euclidean distance metric is %f cycles.\n", Genes, K, (endtime-starttime));
        printf("Medoids:\n");
        for (i = 0; i < K; i++) {
            printf("Medoid %d: ", i + 1);
            for (j = 0; j < Samples; j++) {
