...
/*The mpi code that reads the rice dataset file using MPI I/O. This is the larger dataset */
#include<stdio.h>
#include<stdlib.h>
#include<unistd.h>
#include<stdbool.h>
...
*/
    ...
    if (world_rank == 0) {
        ...
        printf("Total time taken to  read a rice dataset file using MPI with  %d genes with %d clusters is %e seconds.\n", Genes, K, (endtime-starttime)/512000000.0f);
        printf("Total clockcycles taken to read a rice dataset file using MPI with  %d genes with %d clusters  is %f cycles.\n", Genes, K, (endtime-starttime));
