/*This code uses euclidean distance metric and is serial. */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
//Start the clockcycle
  ...
  if(flag2)
  {
   ...
   printf("Total clockcycles taken to run K-Medoids using euclidean distance metric on %d genes with %d clusters is %lld cycles.\n", Genes, K, (finish-start));
   printf("Total time taken to run K-Medoids using euclidean distance metric on %d genes with %d clusters is %e seconds.\n", Genes, K, (finish-start)/512000000.0f);
