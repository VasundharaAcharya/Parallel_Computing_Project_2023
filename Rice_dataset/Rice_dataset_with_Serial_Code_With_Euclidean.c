...
/* The dataset used here is the rice dataset. To cope up with the aimos storage space, we only take 10k Genes and 35 Samples for each of the genes. The distance
metric used here is the euclidean distance metric. After the experimental analysis, we found that the precomputation of distance is not required here. So we do not use 
that logic while computing the medoids */
...
/*After the experimental analysis, we found that euclidean_distance is faster for this application when compared to Manhattan.
That is why we dont precompute the pairwise distance for while using the euclidean distance metric */
