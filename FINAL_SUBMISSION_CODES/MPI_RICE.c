/* RICE DATASET: This code uses KMedoids implemented with Manhattan distance metric to compute the cluster assignments.
As per the requirement of the project, the training file is converted to bin format and is read and written using MPI I/O.
The file reading using MPI works upto 8 MPI ranks as beyond that the file is not equally divisible among the ranks. We also tried to compute the remainder and feed itto last rank.*/
...
/* Uncomment this logic to enable the MPI I/O. MPI I/O to read from the bin file created
          int resultant;
          //For this we need to have the bin file rather than txt file.
          resultant = MPI_File_open(MPI_COMM_WORLD, "input_rice.bin", MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);
...
//The gene data reading from text file
if (world_rank == 0) {
...
//Since I am using all gather and updating the medoids, I dont require the below updation logic.
   ...
   /* for (int a=0; a<K;a++){
    ...
    if (world_rank == 0) {
        ...
        printf("Total time taken to run K-Medoids on %d genes with %d clusters on Rice dataset is %e seconds.\n", Genes, K, (endtime-starttime)/512000000.0f);
        printf("Total clockcycles taken to run K-Medoids on %d genes with %d clusters on Rice dataset  is %f cycles.\n", Genes, K, (endtime-starttime));
        printf("Medoids:\n");
        for (i = 0; i < K; i++) {
            printf("Medoid %d: ", i + 1);
            for (j = 0; j < Samples; j++) {
 ...
 /* MPI fileWRITE takes place here .ALL the files are written successfully*/
