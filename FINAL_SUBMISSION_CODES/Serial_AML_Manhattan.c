...
/* AML DATASET :This code is implementing the serial version of K-Medoids algorithm to cluster the gene samples. To validate the results of the clustering algorithm, we use the 
Silhouette Coefficient. We experimented with different K values to obtain the best K value. The dataset used in this code is Molecular Classification of Cancer: Class Discovery and Class.
In this code we have utilized the Manhattan distance as the distance metrics. We create three output files, clustered gene data, medoids and cluster assignments. The time taken to run 
the K-medoids is recorded along with the time taken to compute the Silhoutte coefficient.*/
